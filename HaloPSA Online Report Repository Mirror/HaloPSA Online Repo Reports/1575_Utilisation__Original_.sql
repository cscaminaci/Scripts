...
(select
      uname as [Engineer]
    , (select round(isnull(sum(timetaken),0),2) from actions where timetaken > 0 and timetaken < 24 and who=uname and convert(date,whe_) =
...
date_id=convert(date,getdate())))
        ) + (select isnull(round(cast(sum(antimetaken) as float)/60,2),0) from areanote where anwho=unum and convert(date,andate) =
    (select date_id from calendar where weekday_id=2 and date_id<=convert(date,getdate()) and date_id>convert(date,getdate()-6) and weekday_id <= (select weekday_id from calendar where 
date_id=convert(date,getdate())))
        )    as [Monday]
