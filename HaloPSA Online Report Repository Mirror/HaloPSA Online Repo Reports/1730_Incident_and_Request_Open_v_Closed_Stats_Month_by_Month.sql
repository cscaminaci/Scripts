select convert(date,dateadd(MONTH, datediff(MONTH, 0, dateoccured), 0)) as [MonthCommencing]
,case when f.requesttype=1 then 'Incident' when f.requesttype=3 then 'Service Request' end as [Ticket type]
,count(a.requesttype) as [Opened]
,count(b.requesttype) as [Closed]
,cast(round(cast(count(b.requesttype)as float)/nullif(cast(count(a.requesttype) as float),0)*100,2) as nvarchar) +'%' as [Percent Closed]
from faults f
...
group by f.requesttype,convert(date,dateadd(MONTH, datediff(MONTH, 0, dateoccured), 0))
