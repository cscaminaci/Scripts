SELECT
    ...
    CAST(FLOOR(COALESCE(CAST(ISNULL(BillableTime, 0) * 60 AS INT), 0) / 60) AS NVARCHAR(9)) + ':' + RIGHT('0' + CAST(FLOOR(COALESCE(CAST(ISNULL(BillableTime, 0) * 60 AS INT), 0) % 60) AS NVARCHAR(2)), 2) AS [Billable Hours],
