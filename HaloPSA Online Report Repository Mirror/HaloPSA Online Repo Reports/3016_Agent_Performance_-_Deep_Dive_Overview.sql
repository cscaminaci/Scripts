...
, (select count (faultid) from faults where requesttypenew=71 and datecleared>@startdate and datecleared<@enddate and clearwhoint=unum and (select top 1 actionnumber from actions where actions.faultid=faults.faultid and actoutcome='Re-Open Request' order by actionnumber)  > 0) as [Manual Re-opens]
...
group by [Agent], [Number of Tickets Responded within SLA], [Resolution Rate %], [Satisfaction Score %], [Tickets Opened], [Assigned Tickets], [Tickets Resolved], [On Time Resolution], [Average Resolution Time (Hours)], [Average Response Time (Hours)], [Manual Re-opens], [Total Actions]
