SELECT
       ...
       ,      round(isnull((select isnull(sum(pphours),0) from prepayhistory where pphours>0 and ppdate>=@startdate
              and ppdate<@enddate and ppareaint=Areaint),0),2) AS [Purchased During]
	   ,      round(isnull((select isnull(sum(pphours),0) from prepayhistory where pphours<0 and ppdate>=@startdate
              and ppdate<@enddate and ppareaint=Areaint),0),2) AS [Expired During]
